...
/** Whether flanking puts this actor flat-footed */
function isFlatFootedFromFlanking(target: ActorPF2e, origin: ActorPF2e): boolean {
    if (!target?.isOfType("creature")) return false;
    const { flanking } = target.attributes;
    return !flanking.flankable
        ? false
        : typeof flanking.flatFootable === "number"
        ? origin.level > flanking.flatFootable
        : flanking.flatFootable;
}
